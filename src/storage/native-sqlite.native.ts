...
export async function getNativeDatabase(): Promise<NativeDatabase | undefined> {
  ...
  if (!nativeDatabasePromise) nativeDatabasePromise = import('expo-sqlite').then(({ openDatabaseSync }) => {
    ...
    nativeDatabase.execSync('CREATE TABLE IF NOT EXISTS app_state (id INTEGER PRIMARY KEY NOT NULL, payload TEXT NOT NULL); CREATE TABLE IF NOT EXISTS app_recovery (id INTEGER PRIMARY KEY AUTOINCREMENT, payload TEXT NOT NULL, created INTEGER NOT NULL); CREATE TABLE IF NOT EXISTS sync_v2_outbox (operation_id TEXT PRIMARY KEY NOT NULL, payload TEXT NOT NULL, created INTEGER NOT NULL);');
