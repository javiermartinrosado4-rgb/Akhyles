...
export default function Progress() {
  ...
  const currentTier = pointsTier(points);
  ...
  return <Page>
    ...
    <Row>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Qué son los A-Points" onPress={() => setShowPointsInfo(value => !value)} style={{ flex: 1 }}>
        <Card><Row><Image source={require("../../assets/brand/icon.png")} style={{ width: 22, height: 22, borderRadius: 5, resizeMode: "contain" }} accessibilityLabel="Akhyles" /><Txt weight="600">A-Points</Txt></Row><Txt size={30} weight="500">{visiblePoints?.toLocaleString(locale, { maximumFractionDigits: 1 }) ?? "—"}</Txt></Card>
      </Pressable>
    </Row>
    {points !== undefined && <Card style={{ gap: 6 }}>
      <Row style={{ justifyContent: "space-between" }}><Txt weight="600">{pointsTier(points).name}</Txt><Txt muted size={12}>{nextTier ? "Meta cercana" : "Nivel máximo"}</Txt></Row>
      <Txt size={20} weight="600">{visiblePoints!.toLocaleString(locale, { maximumFractionDigits: 1 })}{nextTier ? ` / ${nextTier.minimum.toLocaleString(locale)}` : ""} A-Points</Txt>
      {nextTier ? <><View style={{ height: 8, borderRadius: 8, overflow: "hidden", backgroundColor: colors.soft }}><View style={{ width: `${Math.min(100, Math.max(0, (visiblePoints! - currentTier.minimum) / (nextTier.minimum - currentTier.minimum) * 100))}%`, height: "100%", backgroundColor: colors.accent }} /></View><Txt muted size={12}>Te faltan {(nextTier.minimum - visiblePoints!).toLocaleString(locale, { maximumFractionDigits: 1 })} puntos para {nextTier.name}.</Txt></> : <Txt muted size={12}>Tu puntuación puede seguir aumentando dentro de Olimpian.</Txt>}
    </Card>}
    ...
    {showPointsInfo && <Card>
      ...
      <Txt muted size={13}>La escala visual llega a Olimpian desde 150 puntos. El valor numérico no tiene techo; este es tu desglose relativo por grupo:</Txt>
      {score.categories.map(category => <Row key={category.id} style={{ justifyContent: "space-between" }}><Txt size={14}>{category.name}{category.kind === "analogy" ? " · ≈" : ""}</Txt><Txt weight="600">{category.value === undefined ? "Sin datos" : `${presentationPoints(category.value)!.toLocaleString(locale, { maximumFractionDigits: 1 })} pts`}</Txt></Row>)}
      ...
      <Txt muted size={13}>Olimpian comienza en 150 A-Points; por encima de ese valor la puntuación sigue creciendo sin crear niveles nuevos. Las cifras no son percentiles ni categorías de competición.</Txt>
