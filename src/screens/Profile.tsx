...
import { Avatar, AvatarPhotoPicker, AvatarSelect } from "../components/Avatar";
...
import { AchievementPage, CommunityUser } from "../services/community";
...
export default function Profile() {
  ...
  const [section, setSection] = useState<EditSection>("training");
  ...
  useEffect(() => {
    ...
    ]).then(([person, page, notices]) => {
      if (!alive) return;
      setCommunityProfile(person); setCommunityDraft(person); setAchievementCount(page.achievements.length); setNotifications(notices);
  ...
  const openEditor = () => {
    setProfile(state.profile); setPrefs(state.preferences); setCommunityDraft(communityProfile ?? user); setErrors({}); setMessage(""); setSection("training"); setEditing(true);
  };
  const save = async () => {
    ...
    try {
      update(value => ({
        ...value, profile, preferences: prefs, programRevision: planChanged ? 6 : value.programRevision,
  ...
  const updatePrivacy = async (change: Partial<Pick<CommunityUser, "routinePublic" | "progressPublic" | "detailsPublic" | "bodyWeightPublic" | "rankingPublic" | "achievementsPublic">>) => {
    if (!communityDraft || busy) return;
    setBusy(true); setMessage("");
    try {
      const next = { ...communityDraft, ...change };
      if (next.routinePublic) await request("/routines/me", "PUT", exportRoutine(state.routine, state.preferences));
      if (next.progressPublic || next.rankingPublic) await request("/progress/me", "PUT", exportProgress(state, !!next.detailsPublic && !!next.progressPublic, !!next.bodyWeightPublic && !!next.progressPublic));
      const saved = await request<CommunityUser>("/me/privacy", "PATCH", {
        routinePublic: !!next.routinePublic, progressPublic: !!next.progressPublic,
        progressVisibility: next.progressPublic ? next.progressVisibility ?? "friends" : "private",
        detailsPublic: !!next.detailsPublic, bodyWeightPublic: !!next.bodyWeightPublic,
        rankingPublic: !!next.rankingPublic, achievementsPublic: !!next.achievementsPublic,
      });
  ...
  if (editing) return <Page>
    ...
    <View style={{ gap: 8 }}>
      ...
      ] as const).map(([id, label, icon]) => <Pressable key={id} accessibilityRole="button" accessibilityState={{ expanded: section === id }} onPress={() => setSection(id)} style={({ pressed }) => ({ minHeight: 62, paddingHorizontal: 16, borderRadius: 15, borderWidth: 1, borderColor: section === id ? colors.accent : colors.border, backgroundColor: section === id ? colors.accentSoft : colors.surface, flexDirection: "row", alignItems: "center", gap: 12, opacity: pressed ? 0.75 : 1 })}>
    ...
    {section === "community" && <Card>
      ...
      {communityDraft ? <>
        ...
        <CommunityPrivacyToggle label="Compartir logros" description="Publica tiers y marcas personales en Logros" value={!!communityDraft.achievementsPublic} disabled={busy} onChange={() => void updatePrivacy({ achievementsPublic: !communityDraft.achievementsPublic })} />
        ...
        <CommunityPrivacyToggle label="Resumen de progreso" description="Comparte gráficas y marcas con la visibilidad elegida" value={!!communityDraft.progressPublic} disabled={busy} onChange={() => void updatePrivacy({ progressPublic: !communityDraft.progressPublic })} />
        <CommunityPrivacyToggle label="Rutina" description="Visible para amigos mutuos" value={!!communityDraft.routinePublic} disabled={busy} onChange={() => void updatePrivacy({ routinePublic: !communityDraft.routinePublic })} />
    ...
    {section === "app" && <>
      <AccountCard />
      <Card><Txt weight="600" size={20}>Opciones de app</Txt><Txt muted>Idioma, aspecto y preferencias generales.</Txt><Button label="Idioma y apariencia" icon="settings" variant="secondary" onPress={() => router.push("/settings")} /></Card>
  ...
  return <Page>
    ...
    {social && <Card style={{ gap: 6 }}><Txt weight="600">Logros</Txt><Txt muted size={13}>{achievementCount === 1 ? "1 logro compartido" : `${achievementCount} logros compartidos`} · Los nuevos tiers y marcas aparecen en Comunidad.</Txt><Button label="Ver Logros" compact variant="ghost" icon="star" onPress={() => router.push("/community")} /></Card>}
    ...
    <Modal visible={notificationsOpen} animationType="slide" onRequestClose={() => setNotificationsOpen(false)}>
      ...
      {!user ? <Card><Txt weight="600">Conecta Comunidad para ver tus avisos</Txt><Txt muted>Los seguimientos y novedades de tu perfil aparecerán aquí.</Txt><Button label="Conectar Comunidad" icon="users" variant="secondary" onPress={() => { setNotificationsOpen(false); router.push("/community"); }} /></Card> : notifications.length ? notifications.map(item => <Pressable key={item.id} accessibilityRole="button" onPress={() => { setNotificationsOpen(false); router.push({ pathname: "/community", params: { profile: item.actorId } }); }} style={({ pressed }) => ({ paddingVertical: 14, paddingHorizontal: 2, gap: 4, borderBottomWidth: 1, borderBottomColor: colors.border, opacity: pressed ? 0.7 : 1 })}>
        <Row style={{ justifyContent: "space-between", alignItems: "flex-start" }}><Txt style={{ flex: 1 }}><Txt weight="600" translate={false}>@{item.handle}</Txt> ha empezado a seguirte.</Txt><Txt muted size={11}>{item.read_at ? "Vista" : "Nueva"}</Txt></Row><Txt muted size={12}>{new Date(item.created).toLocaleString(locale)}</Txt>
